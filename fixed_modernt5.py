...
_CHECKPOINT_FOR_DOC = "your-org/ModernT5-base"
_CONFIG_FOR_DOC = "ModernT5Config"
MODERNBERT_START_DOCSTRING = ModernBertModel.__doc__.split("Parameters:")[0] if ModernBertModel.__doc__ else ""
...
class ModernT5Config(ModernBertConfig):
    ...
    def __init__(
        ...
        decoder_local_attention: Optional[int] = -1,
    ...
    def forward(
        ...
        if self.is_self_attn:
            ...
            if past_key_value is not None:
                # Check if past_key_value contains valid tensors
        ...
        if use_cache and self.is_self_attn:
...
class ModernT5PreTrainedModel(ModernBertPreTrainedModel, GenerationMixin):
...
class ModernT5Stack(ModernT5PreTrainedModel, GenerationMixin):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if input_shape[-1] > 1 or past_key_values_length > 0:
    ...
    def forward(
        ...
        return BaseModelOutputWithPastAndCrossAttentions(
            ...
            attentions=all_self_attns or None,
            cross_attentions=all_cross_attns or None,
        )
...
class ModernT5Model(ModernT5PreTrainedModel, GenerationMixin):
    ...
    def set_input_embeddings(self, new_embeddings: nn.Embedding):
        if self.shared is not None:
            ...
            if self.decoder.embed_tokens is None:
...
class ModernT5ForConditionalGeneration(ModernT5PreTrainedModel, GenerationMixin):
    ...
    def set_input_embeddings(self, new_embeddings):
        ...
        if self.lm_head is not None and self.config.tie_word_embeddings:
            self.tie_weights()
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, inputs_tensor: torch.Tensor, model_kwargs, model_input_name: Optional[str] = None, generation_config=None, **kwargs
    ):
        if "encoder_outputs" not in model_kwargs:
            ...
            if "attention_mask" not in encoder_kwargs:
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        **kwargs,
        ...
        _bos_token_id = bos_token_id if bos_token_id is not None else self.config.bos_token_id
        if _bos_token_id is None: _bos_token_id = _decoder_start_token_id
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if decoder_position_ids is None and decoder_input_ids is not None:
    ...
    def prepare_inputs_for_generation(
        ...
        cache_position=None,  # Add this for modern Transformers compatibility
        **kwargs
    ):
        if past_key_values is not None and len(past_key_values) > 0:
            if cache_position is not None:
                past_length = cache_position[0].item()  # Modern way: first position is the cache start
            else:
                # Fallback to old way (check KV tensor shape)
                if isinstance(past_key_values[0], tuple) and len(past_key_values[0]) > 0:
                    past_length = past_key_values[0][0].shape[2]
                else:
                    past_length = 0

            # Robust slicing like in T5: handle if input is full seq or just last token
            if decoder_input_ids.shape[1] > past_length:
                decoder_input_ids = decoder_input_ids[:, past_length:]
            else:
                decoder_input_ids = decoder_input_ids[:, -1:]
        else:
            past_length = 0  # No cache, full input

        # If your decoder uses absolute positions (BERT-style), prepare them here if not provided
        if decoder_position_ids is None and cache_position is not None:
            # Use cache_position for positions (modern way)
            decoder_position_ids = cache_position
        elif decoder_position_ids is None:
            # Fallback: create on the fly from decoder_attention_mask or seq length
            decoder_position_ids = torch.arange(
                past_length, past_length + decoder_input_ids.shape[-1], dtype=torch.long, device=decoder_input_ids.device
            ).unsqueeze(0).expand(decoder_input_ids.shape[0], -1)

        # Expand masks if needed (standard T5-like handling)
        if attention_mask is not None:
            # Assuming encoder attention_mask; pad if necessary
            pass  # Add logic if your encoder needs it

        if decoder_attention_mask is not None and past_key_values is not None:
            # Extend decoder mask for new tokens
        ...
        return {
            ...
            "use_cache": use_cache,
            "cache_position": cache_position,  # Pass it forward if your forward() uses it
        }

    # def prepare_inputs_for_generation(
    #     self, decoder_input_ids, past_key_values=None, attention_mask=None,
